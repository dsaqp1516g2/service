...
public interface ProjectDAOQuery {
    ...
    public final static String GET_PROJECTS_FROM_MEMBER = "select hex(p.id) as id, p.name, p.description, p.repo_url, p.creation_timestamp, hex(p.admin_userid) as adminId, u.fullname as adminName from projects p, users u, user_projects m where m.userid=unhex(?) and p.id=m.projectid and u.id=p.admin_userid";
}
